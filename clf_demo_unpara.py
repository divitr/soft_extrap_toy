...
def generate_pt(num: int):
    ...
    keras_model.fit(f_train_sb, train_y_sb, epochs=100, batch_size=500, verbose=1,shuffle=True)
    ...
    keras_model_.fit(f_train_tb, train_y_tb, epochs=100, batch_size=500, verbose=1,shuffle=True)
...
def generate_f1(pt_source: np.array, pt_target: np.array , pt_background: np.array, num): 
    ...
    f1_background = np.add(f1_background, (-1)*np.multiply(pt_background,(np.sin(theta)-0.5)))
...
def generate_dataframe(f1_source_:np.array, f1_background_:np.array, f2_source_:np.array, f2_background_:np.array):#, pt_source_:np.array, pt_background_):
    id = np.ones([len(f1_source_),1])
    id = id.astype(int)
    id = id.flatten()
    id = pd.DataFrame({'class_id':id})
    id_b = np.zeros([len(f1_background_),1])
    id_b = id_b.astype(int)
    id_b = id_b.flatten()
    id_b = pd.DataFrame({'class_id':pd.Series(id_b)})
    class_id = pd.concat([id,id_b])
    ...
    #pt.reset_index(drop=True, inplace=True)
    #f2 = pd.concat([f2_source, f2_background])
    f = pd.DataFrame({'x1':f1['x1'],'x2':f2['x2'],'class_id': class_id['class_id']}, index=range(len(class_id)))
    return f
...
if __name__ == "__main__":
    ...
    for i_num in [0.1, 0.5, 1, 2, 4, 8, 20, 50, 100, 500]:
        ...
        #weight_sb = np.load('/mnt/d/DW_project/weight_sb.npy')
        #weight_tb = np.load('/mnt/d/DW_project/weight_tb.npy')
        #weight_fit_sb = np.append(weight_sb,np.ones([1,num_noi]))
        #weight_fit_tb = np.append(weight_tb,np.ones([1,num_noi]))
        ...
        pt_source, pt_target, pt_background, weight_sb, weight_tb = generate_pt(50000)
        f1_source, f1_target,f1_background = generate_f1(pt_source, pt_target, pt_background, 50000)
        f2_source, f2_target,f2_background = generate_f2(pt_source, pt_target, pt_background, 50000)
        ...
        plt.legend(loc='best')
        plt.savefig('/mnt/d/DW_project/num/0530/' + str(num) + 'unpara' + '.png')
        plt.close()
